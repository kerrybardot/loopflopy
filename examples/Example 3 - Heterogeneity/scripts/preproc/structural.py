...
def prepare_strat_column(structuralmodel):
    ...
    structuralmodel.nlg = nlg
    
def prepare_geodata(structuralmodel, spatial, dem_tif, control = False, truth = False,Fault = True, extent = None):  

    # Excel sheet needs to have columns: id, x, y, type, Ground then Unit names in strat order
...
# ---------- Prepare borehole data ----------------
    ...
    for i in range(len(data_list)): #iterate for each row
        ...
        data_type = data_list[i][df.columns.get_loc("type")]
        
        #-----------RAW DATA-----------------------
        if data_type == 'Raw': ### Z VALUES: Ground (mAHD), Else: (mBGL)
            
            boreid = data_list[i][df.columns.get_loc("id")]
            x, y = data_list[i][df.columns.get_loc("x")], data_list[i][df.columns.get_loc("y")]
            ...
            lith_col_start = df.columns.get_loc("Ground")+1

            for j in range(lith_col_start, df.shape[1]-1): #iterate through each formation 
                if isinstance(data_list[i][j], numbers.Number) == True:  # Add lithology  
                    ...
                    val       = structuralmodel.strat.val[count]                   # designated isovalue
                    unit      = structuralmodel.strat.unit[count]                  # unit 
                    feature   = structuralmodel.strat.sequence[count]              # sequence
        ...
        if control:
            if data_type == 'Control': ### Z VALUES: mAHD
                ...
                x, y = data_list[i][df.columns.get_loc("x")], data_list[i][df.columns.get_loc("y")]
                #groundlevel = data_list[i][5] 
                groundlevel = find_elevation(x, y, spatial, dem_tif) 
...
def create_structuralmodel(structuralmodel, nelements = 1e4, Fault = True):
    ...
    Leed1    = model.create_and_add_foliation("Leed1", nelements=nelements, buffer=0.1)    
    Leed1_UC = model.add_unconformity(Leed1, structuralmodel.strat[structuralmodel.strat.unit == 'Kwlp'].val.iloc[0]) # Make a new UC
    Leed2    = model.create_and_add_foliation("Leed2", nelements=nelements, buffer=0.1)    # Make a new UC
